...
snap_folders = [PATH_TO_DATA_FROM_EXEC + "snap_ad-hoc_histo_" + str(i) + "/" for i in ex]
...
for snap_folder in snap_folders:
    ...
    histo_folder = str(snap_folder).replace("snap_ad-hoc_histo", "histo_ad-hoc_histo")
    ...
    print(f"\nAd-hoc ex {ex[i]} histo extraction completed in {elapsed_time:.4f} seconds.\n")
