...
void SEMproxy::run()
{
  ...
  if (save_sismos)
  {
    ...
    for (size_t i = 0; i < sismosPoints.size(); i++)
    {
      ...
      std::string filename =
          "sismo_" + std::to_string(index_node_sismos) + ".csv";
      ...
      std::cout << "✔ Fichier créé : " << filename << " pour sismo #" << i
       ...
       indexTimeSample++)
  {
    ...
    if (save_snapshot && indexTimeSample % snapInterval == 0)
    {
      ...
      if (save_sismos)
      {
        ...
        {
          int node = sismosNodeIndex[i];  // node FEM pour ce sismo
        ...
        // float pression = pnGlobal(index_node_sismos, i2);

        // std::string filename = "/sismo_" + std::to_string(index_node_sismos)
        // + ".csv"; std::ofstream out(filename, std::ios::app); if
        // (!out.is_open())
        // {
        //     std::cerr << "Error: could not open sismo file " << filename <<
        //     std::endl; return;
    ...
    float kerneltime_ms = time_point_cast<microseconds>(totalComputeTime)
                              .time_since_epoch()
                              .count();
    float outputtime_ms = time_point_cast<microseconds>(totalOutputTime)
                              .time_since_epoch()
                              .count();

    cout << "------------------------------------------------ " << endl;
    cout << "\n---- Elapsed Kernel Time : " << kerneltime_ms / 1E6
         << " seconds." << endl;
  ...
  void SEMproxy::parsePointSismos(const std::string& filename)
  {
    ...
    sismosPoints.clear();  // vider l'ancien contenu
    ...
    std::cout << "✔ " << sismosPoints.size() << " sismomètres chargés depuis "
              << filename << std::endl;
  }
